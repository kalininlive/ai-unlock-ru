...
const STATES = {
  ...
  ruDirect: {
    iconBase: 'icons/direct',
    badgeColor: '#3a7bd5',
    tooltipFn: ({ host, viaRuProxy }) => viaRuProxy
      ? `Gemini Unblock — ${host} — RU service, routed via RU proxy`
      : `Gemini Unblock — ${host} — RU service, always direct (no RU proxy set)`,
  },
  error: {
 ...
 * 'off' | 'routed' | 'direct' | 'ruDirect' | 'error'.
